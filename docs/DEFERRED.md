...
| DEF-033 | Short parent videos | research | Some parents prefer watching; a generated proof of concept now exists | Script from existing briefing fields. Pacing: short beats + punctuation + inserted silence (Kokoro has no SSML). Visuals: recurring SVG guide + ten-frame / part–whole kit. Later options, not now: Remotion timeline, Rive guide states, SSML voice (ElevenLabs / Azure). Do not generate classroom footage. Re-render after language fixes. |
