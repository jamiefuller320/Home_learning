...
| DEF-033 | Short parent videos | research | Some parents prefer watching; a generated proof of concept now exists | Script from existing briefing fields as a concise lesson + task outline + page handoff (not a full page reading). Voice: Kokoro British `bf_isabella` near speed 1.05 (no SSML — punctuation + short beats). Visuals: recurring SVG guide + ten-frame / part–whole kit. Later options, not now: Remotion timeline, Rive guide states, SSML voice (ElevenLabs / Azure). Do not generate classroom footage. Re-render after language, script-shape, or voice changes. |
