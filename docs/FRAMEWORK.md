...
2. **Keep the school method.** If the suggestion would teach a rival method, drop a needed term, or make the maths vaguer, decline it (`npx tsx scripts/language-notes.ts decline <id>`).
...
4. **Close the row.** `done` after the topic file changes; `declined` when we will not change the pack, with the reason in the PR.
